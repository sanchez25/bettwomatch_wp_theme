...
<section class="page-block-news">
  <div class="page-row">
    <div class="page-block-content">
      ...
      <div class="page-text">
        <div class="page-text-news">
          <?php if ($items = get_posts(array('numberposts' => 20, 'post_type' => 'Bonuses'))) {
            foreach($items as $item) {
              $bonus = get_field('bonus-card', $item->ID);
              $linkbonus = get_field('link-bonus', $item->ID);
                echo '<div class="news-item-page">';
                  echo '<a class="news-item-link" href="'.get_permalink($item->ID).'">';
                    echo '<div class="bonuses-block-image">';
                      echo get_the_post_thumbnail($item->ID);
                    echo '</div>';
                    echo '<div class="news-block-title">';
                      echo '<h3 class="title-bonuses-h3">'.$item->post_title.'</h3>';
                    echo '</div>';
                    echo '<div class="bonus-block-descr">';
                      echo '<span class="title--h3">'.$bonus.'</span>';
                    echo '</div>';
                  echo '</a>';
                  echo '<div class="bonuses-block-button page">';
                    echo '<a class="bookmmaker-main-button pd bonus" href="'.get_permalink($item->ID).'" tabindex="0">Подробнее</a>';
                    echo '<a class="bookmmaker-main-button pl bonus" href="'.$linkbonus.'" tabindex="0">Получить бонус</a>';
                  echo '</div>';
                echo '</div>';
              }
            }
